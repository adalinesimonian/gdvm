...
gdvm search --limit 0 --filter 4 | grep 4.3-stable
TEST_SCRIPT
...
if gdvm search --cache-only --limit 0 --filter 4 | grep -q 4.3-stable; then
    echo "cache-only search unexpectedly found releases with empty cache"
    [[ -f "$backup" ]] && mv "$backup" "$cache"
    exit 1
fi

gdvm search --refresh --limit 0 --filter 4 | grep 4.3-stable
