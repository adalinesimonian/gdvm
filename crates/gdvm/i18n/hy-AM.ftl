...
error-config-path-empty = Ուղին չի կարող դատարկ լինել։
error-config-path-file = Ուղին վերաբերում է ֆայլին, а ոչ գրացուցակին։ { $path }
error-config-path-reserved = Ուղին պահպանված է gdvm-ի ներքին օգտագործման համար։ { $path }
error-config-path-overlap = Կոնֆիգուրացված ուղիները չի կարող համընկնել։ { $key } ({ $path })
error-config-dir-not-empty = Մերժվում է օգտագործել { $path } ուղին, քանի որ այն արդեն содержит файлы։ Ընտրեք դատարկ գրացուցակ կամ նոր ուղի։
