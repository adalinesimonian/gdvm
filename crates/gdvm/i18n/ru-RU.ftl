...
error-copy-file-failed = Не удалось скопировать файл: { $error }
error-move-file-failed = Не удалось переместить файл: { $error }
...
warning-shortcut-macos-not-supported = На данный момент, создание ярлыков в MacOS не поддерживается, так что ярлык не будет создан.