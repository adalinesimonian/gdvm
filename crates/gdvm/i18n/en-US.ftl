...
error-config-path-empty = Path cannot be empty.
error-config-path-file = Path points to a file, not a directory: { $path }
error-config-path-reserved = Path is reserved for gdvm internals: { $path }
error-config-path-overlap = Configured paths must not overlap: { $key } ({ $path })
error-config-dir-not-empty = Refusing to use path { $path } because it already contains files. Choose an empty directory or a new path.
